...
#else /* _WIN32 */
#define RIME_DLL
/* moocat: on non-Windows static lib builds, the original empty
   `RIME_API` macro left function definitions with default C++
   linkage — name-mangled, unreachable from `.m` / `.c` consumers.
   `extern "C"` keeps the C-callable symbol names that rime_api.cc
   defines (RimeSetup, RimeConfigClose, etc.) so static-link
   consumers (LibrimeKit's ObjC wrapper) can find them.
   The header-level `extern "C" {` block already covers
   declarations; this fixes definitions to match. */
#ifdef __cplusplus
